...
% GRIPPER ajoute un corps rigide representant un gripper a
...
% modele dynamique du robot.
%
% Entrees :
%   robot : Objet RigidBodyTree
%   gripper_length : Longueur du gripper, decalage en Z (m) du derniere axe (tool0)
%   gripper_mass : Masse du gripper (kg) pour le modele dynamique
%
% Sortie :
%   robot : Model du robot modifie avec le corps 'gripper_tip' ajoute
%
    ...
    % Inertie approchee comme un cylindre mince de rayon r = 0.02 m. Voir draw_gripper.m pour reference
    r_grip = 0.02;

    Ixx = (1/12) * gripper_mass * (3*r_grip^2 + gripper_length^2); % (cours 5, page 17)
    Iyy = Ixx;
    Izz = (1/2) * gripper_mass * r_grip^2;
    gripperBody.Inertia = [Ixx, Iyy, Izz, 0, 0, 0];

    % Ajout du tyoe d'articulation pour le corps du gripper
