function plot_commande_impedance_verticale(robot, t_vec, x_d, x_c_hist, x_hist, err_hist, tau_hist, ...
                                           Fext_hist, q_hist, z_table, gripper_length, N_frames)
% plot_commande_impedance_verticale affiche les resultats de la commande
% d'impedance verticale sur une table.
    ...
    %% 1) Trajectoire 3D
    figure('Name', 'Impedance verticale - Trajectoire 3D');
    ...
    ylabel('Y (m)');
    zlabel('Z (m)');
    title('Commande d''impedance verticale sur une table');
    legend('Trajectoire desiree', 'Trajectoire corrigee', 'Trajectoire reelle', 'Table', ...
           'Location', 'best');

    %% 2) Positions cartesiennes
    figure('Name', 'Impedance verticale - Positions');
    ...
    %% 3) Erreur
    figure('Name', 'Impedance verticale - Erreur');
    subplot(2,1,1);
    ...
    fprintf('\nErreur maximale de suivi (impedance) : %.4f mm\n', ...
        1000 * max(vecnorm(err_hist,2,1)));
    fprintf('Erreur finale de suivi   (impedance) : %.4f mm\n', ...
        1000 * norm(err_hist(:,end)));

    %% 4) Force de contact
    figure('Name', 'Impedance verticale - Force de contact');
    plot(t_vec, Fext_hist(3,:), 'LineWidth', 1.5);
    grid on;
    xlabel('Temps (s)');
    ylabel('F_z (N)');
    title('Force normale de contact');

    %% 5) Couples articulaires
    figure('Name', 'Impedance verticale - Couples articulaires');
    for j = 1:6
        subplot(3,2,j);
    ...
    figure('Name', 'Animation - Commande d''impedance verticale');
    ...
    title('Animation de la commande d''impedance verticale');
    xlabel('X (m)');
    ylabel('Y (m)');
    zlabel('Z (m)');
    grid on;
    ...
    zmin_plot = min([z_table, min(x_hist(3,:)), min(x_c_hist(3,:)), min(x_d(3,:))]) - 0.95;
    zmax_plot = max([max(x_hist(3,:)), max(x_c_hist(3,:)), max(x_d(3,:))]) + 0.95;
    ...
    hold off;
    fprintf('Animation de la commande d''impedance verticale terminee.\n');
end